...
### Aligné sur le Cahier de Vision V2
...
## 4. Modèle Pédagogique V2

### 4.1 Structure Standard Obligatoire d'une Compétence

Chaque compétence doit respecter une structure stricte en **13 points** :
...
| 4 | **Prérequis** | ✅ | Liens vers compétences requises |
| 5 | **Mise en contexte** | ✅ | Motivante pour l'apprenant |
| 6 | **Explication théorique** | ✅ | Contenu didactique |
| 7 | **Exemples guidés** | ✅ | Problèmes résolus pas à pas |
| 8 | **Exercices progressifs** | ✅ | Du simple au complexe |
| 9 | **Corrections feedback** | ✅ | Explications sur les erreurs |
| 10 | **Auto-évaluation** | ⚪ | Questions réflexives |
| 11 | **Synthèse** | ✅ | À retenir |
| 12 | **Enrichissements** | ⚪ | Optionnels |
| 13 | **Traçabilité** | ✅ | Historique interactions |

### 4.2 Contenu Officiel vs Contenu Utilisateur
...
Matières (subjects)     → Plateforme UNIQUEMENT ❌ Utilisateurs
        │
        ▼
Modules (domains)       → Plateforme + Utilisateurs ✅
        │
        ▼
Compétences (skills)    → Plateforme + Utilisateurs ✅
        │
        ▼
Exercices               → Plateforme + Utilisateurs + IA ✅
```

| Type contenu | Créateur | Validation | Visibilité |
|--------------|----------|------------|------------|
| **Officiel** | Plateforme | Pré-validé | Public |
| **Utilisateur** | Parent/Enseignant | Validation IA implicite | Privé/Partageable |
| **IA généré** | Système | Validation schéma + review | Selon source |

**Validation implicite IA** : L'IA vérifie que le contenu généré :
- Correspond à la matière sélectionnée
- Respecte le cadre pédagogique
- Ne sort pas du domaine disciplinaire

### 4.3 Méthodes Pédagogiques

| Méthode | Code | Description |
|---------|------|-------------|
| **Standard** | `standard` | Approche classique structurée |
| **Montessori** | `montessori` | Découverte autonome, manipulation |
| **Singapour** | `singapore` | Concret → Imagé → Abstrait |
| **Ludique** | `gamified` | Défis, récompenses, narration |

Règles :
- Méthode par défaut configurable par matière (admin)
- Utilisateur libre de choisir sa méthode
- Méthode explicitement indiquée dans les métadonnées
- Ne modifie jamais les objectifs pédagogiques

### 4.4 Modèle Freemium — Quotas

| Fonctionnalité | Gratuit | Premium |
|----------------|---------|---------|
| Modules créés/mois | 3 | Illimité |
| Compétences créées/mois | 10 | Illimité |
| Exercices IA par compétence | 10 (tokens plateforme) | Illimité |
| Personnalisation avancée | ❌ | ✅ |
| BYOK (clé API perso) | ✅ | ✅ |

**Règle des 10 exercices** :
- Jusqu'à 10 exercices/compétence avec tokens plateforme
- Au-delà : tokens personnels ou clé API requise
- Message explicite lors du changement de mode
...
#### Schéma principal — Tables existantes
...
#### Schéma V2 — Nouvelles tables

```sql
-- =============================================
-- STRUCTURE COMPÉTENCE COMPLÈTE (V2)
-- =============================================

-- Contenu structuré d'une compétence (13 points)
...
-- =============================================
-- CONTENU UTILISATEUR (V2)
-- =============================================

-- Modules créés par les utilisateurs
...
ALTER TABLE subjects ADD COLUMN IF NOT EXISTS default_method TEXT DEFAULT 'standard';

-- =============================================
-- QUOTAS ET FREEMIUM (V2)
-- =============================================
...
CREATE INDEX idx_skill_content_skill ON skill_content(skill_id);
CREATE INDEX idx_skill_examples_skill ON skill_examples(skill_id);
CREATE INDEX idx_user_modules_subject ON user_modules(subject_id);
CREATE INDEX idx_user_modules_creator ON user_modules(created_by);
CREATE INDEX idx_user_skills_creator ON user_skills(created_by);
...
## 14. Offline & PWA (Phase 2)

### 14.1 Service Worker Strategy
...
## 16. Estimations & Planning

### 16.1 Phase 1 — MVP (12 semaines)
